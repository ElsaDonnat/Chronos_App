...
- Zoom-aware clustering: base grid = 25 SVG-unit cells, scales inversely with zoom level (`CLUSTER_GRID / zoom`). Uses debounced scale (150ms) to avoid recalculation during pinch gestures. Cluster drill-down: tapping a cluster auto-zooms to separate pins; flat list shown as fallback at 3×+ zoom.
- Pinch-zoom via CSS `transform: scale()` + custom touch pan handler (max 4×); desktop wheel zoom via `onWheel`; double-tap to zoom (2× centered on tap point, double-tap again resets). **Semantic zoom**: all SVG attributes (pin radii, stroke widths, text sizes) are divided by `scale` so elements stay constant screen-size at any zoom level. Continent labels fade out at 2×+. Pin title labels (with parchment halo for readability) appear at 2×+ zoom for learned non-cluster pins.
- Fullscreen mode: 280% width SVG inside a scrollable container, centered on Europe/Middle East; auto-scrolls to selected region; swipe-down gesture to dismiss (pull-down > 80px with rubber-band physics and visual hint pill). **Mini-map**: small overview in bottom-left corner showing viewport position, click to navigate, zoom-aware via inverse CSS transform math. **First-time hints**: gesture tutorial overlay (scroll + pinch) with portrait orientation suggestion, auto-dismiss 4s, localStorage flag `chronos-map-fs-tutorial`.
...
**Pin interaction**: single pin → event popup card; cluster pin → auto-zooms to separate pins (drill-down), falls back to expandable event list at high zoom. All learned events (in `seenEvents`) appear on the map. Selecting an event highlights its country (gold fill) using `EVENT_COUNTRY_MAP` (event ID → ISO country code, generated by the data pipeline). Selecting a connected pin draws curved bezier arcs (`ConnectionArcs` component) to related events via `EVENT_CONNECTIONS` data — category-colored, solid for learned, dashed for unlearned.
...
Casual endless MCQ quiz in the Challenge hub. 50 curated trivia questions tied to specific event cards. Each fun fact has a question, 4 options (1 correct + 3 wrong), and a "Did you know?" explanation. Facts cover surprising details NOT found in card descriptions.
