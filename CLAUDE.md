...
**Pin interaction**: single pin → event popup card; cluster pin → expandable event list. Only learned events (in `seenEvents`) appear on the map. Selecting an event highlights its country (gold fill) using `EVENT_COUNTRY_MAP` (event ID → ISO country code, generated by the data pipeline).
...
- Per-country highlighting on event selection works; no hover states yet
